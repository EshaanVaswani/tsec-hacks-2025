...
const SearchBar: React.FC<SearchBarProps> = ({
  ...
  const defaultSearchFunction = async (
    ...
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/api/user/search`,
        {
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    setIsLoading(true);
    try {
      const searchResults = await (searchFunction || defaultSearchFunction)(
        searchQuery
      );
  ...
  return (
    <div className="relative w-full max-w-md mb-1">
      <Input
        type="text"
        placeholder="Search lawyers by username"
        value={query}
        onChange={(e) => setQuery(e.target.value)}
        onFocus={() =>
          query.length > 1 && results.length > 0 && setIsResultsVisible(true)
        }
